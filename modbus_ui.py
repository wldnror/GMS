...
class ModbusUI:
    ...
    def __init__(self, parent, num_boxes, gas_types, alarm_callback):
        ...
        # TFTP/FW/ZERO/RST 지원 여부 (초기엔 True, 통신/프로브 중 자동 판단/갱신)
        self.tftp_supported = [True] * num_boxes
        # FW 상태 레지스터(40023/40024) 지원 여부
        self.fw_status_supported = [True] * num_boxes
        # ▼ 감지기 모델(예: 40030~) 지원 여부 (미지원이면 절대 읽기 시도 X)
        self.model_supported = [True] * num_boxes
    ...
    def connect(self, i):
        ...
        if ip and ip not in self.connected_clients:
            ...
            if self.connect_to_server(ip, client):
                ...
                # 기본값: 먼저 True로 두고, 같은 연결(같은 client)로 capability probe 확정
                self.tftp_supported[i] = True
                self.fw_status_supported[i] = True
                self.model_supported[i] = True
                self.last_fw_status[i] = None
                self.box_states[i]["fw_upgrading"] = False

                # ★ 같은 소켓(같은 client)로만 probe (장비가 1소켓만 허용하는 경우도 안전)
                try:
                ...
                # ▼ 모델 문자열은 연결 직후 "한 번만" (미지원이면 즉시 disable하고 끝)
                threading.Thread(
                    target=self.delayed_load_model_from_device,
                    args=(i, 0.8),
                    daemon=True,
                ).start()
    ...
    def detect_device_capabilities(self, ip: str, box_index: int):
        """
        ★ 중요 변경:
        - 임시 클라이언트(두 번째 TCP 연결) 만들지 않고,
          '이미 연결된 client'로 probe 한다.
        - 일부 장비는 Modbus 연결을 1개만 허용해서, 두 번째 connect가 'Connection refused'가 나올 수 있음.
        """
        client = self.clients.get(ip)
        lock = self.modbus_locks.get(ip)
        if client is None or lock is None:
            # 연결 객체가 없으면 확장 기능은 모두 OFF로
            self.fw_status_supported[box_index] = False
            self.tftp_supported[box_index] = False
            self.model_supported[box_index] = False
            return
        ...
        try:
            with lock:
                rr_base = client.read_holding_registers(start_address, BASE_REG_COUNT)
            ...
            # 2단계: 확장 포함 24개 읽기 시도
            with lock:
                rr_ext = client.read_holding_registers(start_address, 24)

            if isinstance(rr_ext, ExceptionResponse) or rr_ext.isError():
                self.console.print(
                    f"[FW] box {box_index} ({ip}) : capability probe 결과 → "
                    f"40023/40024 읽기 에러 → 구형 장비 (FW/TFTP/ZERO/RST/MODEL 비활성화). ({rr_ext})"
                )
                self.fw_status_supported[box_index] = False
                self.tftp_supported[box_index] = False
                self.model_supported[box_index] = False
            ...
            if len(regs_ext) >= 24:
                ...
                # 모델은 (프로토콜마다 다를 수 있어) 일단 True로 두고, 실제 모델 읽기 실패 시 disable
                self.model_supported[box_index] = True
            else:
                self.console.print(
                    f"[FW] box {box_index} ({ip}) : capability probe 결과 → "
                    f"24개 미만 응답(={len(regs_ext)}) → 구형 장비 (FW/TFTP/ZERO/RST/MODEL 비활성화)."
                )
                self.fw_status_supported[box_index] = False
                self.tftp_supported[box_index] = False
                self.model_supported[box_index] = False
            ...
            # probe 중 에러가 나도 연결을 끊지 말고 “확장 기능만 OFF”로 처리
            self.console.print(
                f"[FW] box {box_index} ({ip}) : capability probe exception → "
                f"확장 기능 OFF 처리 (연결은 유지). {e}"
            )
            self.fw_status_supported[box_index] = False
            self.tftp_supported[box_index] = False
            self.model_supported[box_index] = False

    # -------------------- 감지기 모델 읽기 (안전: 미지원이면 절대 시도 X) --------------------

    def delayed_load_model_from_device(self, box_index: int, delay: float = 1.0):
        time.sleep(delay)
        try:
            self.load_model_from_device(box_index)
        except Exception as e:
            # 절대 예외로 연결 루프를 깨지 않음
            self.console.print(f"[MODEL] (ignore) delayed model read fail box {box_index}: {e}")

    def load_model_from_device(self, box_index: int):
        """
        감지기 모델 문자열을 읽어서 우측 상단(version 라벨) 옆에 같이 표시.
        - 미지원/에러면 model_supported=False로 내려서 이후 절대 재시도하지 않음
        - 읽기 실패가 전체 연결을 끊거나 reconnect를 유발하면 안 됨
        """
        if not self.model_supported[box_index]:
            return
        ...
        # ★ 여기 주소/길이는 장비 프로토콜에 맞게 조정 가능
        # 40030부터 10워드(20바이트) 정도를 ASCII로 받는다고 가정
        ...
        try:
            ...
            if isinstance(rr, ExceptionResponse) or rr.isError():
                msg = str(rr)
                self.console.print(f"[MODEL] box {box_index} read 40030~ error → disable: {msg}")
                self.model_supported[box_index] = False
                return

            regs = getattr(rr, "registers", []) or []
            if len(regs) < 1:
                self.console.print(f"[MODEL] box {box_index} read 40030~ empty → disable")
                self.model_supported[box_index] = False
                return

            # 워드 -> 바이트(상위바이트, 하위바이트) -> ASCII
            b = bytearray()
            for w in regs:
                b.append((w >> 8) & 0xFF)
                b.append(w & 0xFF)

            # NULL(0x00) 이후는 문자열 끝으로 처리
            if 0 in b:
                b = b[: b.index(0)]

            model = b.decode("ascii", errors="ignore").strip()
            if not model:
                self.console.print(f"[MODEL] box {box_index} model empty → disable")
                self.model_supported[box_index] = False
                return

            self.box_states[box_index]["model_text"] = model
            self.ui_update_queue.put(("version", box_index, self.box_states[box_index].get("last_version_value") or 0))
            self.console.print(f"[MODEL] box {box_index} model='{model}'")

        except Exception as e:
            msg = str(e)
            # “No response received/Invalid Message” 류는 장비가 해당 주소를 아예 지원 안 하는 케이스가 많음
            if "No response received" in msg or "Invalid Message" in msg or "Unable to decode response" in msg:
                self.console.print(f"[MODEL] box {box_index} 40030~ not supported → disable ({msg})")
                self.model_supported[box_index] = False
                return

            # 기타도 안전하게 disable
            self.console.print(f"[MODEL] box {box_index} model read exception → disable ({e})")
            self.model_supported[box_index] = False
        ...
        while not stop_flag.is_set():
            try:
                ...
                if self.box_states[box_index].get("fw_upgrading", False):
                    self.console.print(f"[FW] box {box_index} disconnected during upgrade (expected).")
                ...
                if self.fw_status_supported[box_index] and "40001~40024" in msg:
                    self.console.print(
                        ...
                        f"FW/TFTP/ZERO/RST/MODEL 기능을 비활성화하고 통신은 계속 진행합니다. ({e})"
                    )
                    self.fw_status_supported[box_index] = False
                    self.tftp_supported[box_index] = False
                    self.model_supported[box_index] = False
                ...
                if new_client.connect():
                    ...
                    try:
                        ...
                        self.console.print(
                            ...
                            f"model_supported={self.model_supported[box_index]})"
                        )
                    except Exception as e:
                        self.console.print(
    ...
    def _do_firmware_upgrade(self, box_index: int):
        ...
        try:
            ...
            self.parent.after(0, lambda e=e: messagebox.showerror("FW", f"TFTP 디렉터리 생성에 실패했습니다.\n{e}"))
            return
        ...
        try:
            ...
            self.parent.after(0, lambda e=e: messagebox.showerror("FW", f"FW 파일 복사에 실패했습니다.\n{e}"))
            return
        ...
        try:
            with lock:
                ...
                try:
                    ...
                    if isinstance(r2, ExceptionResponse) or r2.isError():
                        ...
                        self.parent.after(
                            0,
                            lambda r2=r2: messagebox.showerror("FW", f"장비에 FW 시작 명령을 쓰는 데 실패했습니다.\n{r2}"),
                        )
                        return
            ...
            self.parent.after(0, lambda e=e: messagebox.showerror("FW", f"FW 업그레이드 중 오류가 발생했습니다.\n{e}"))
    ...
    def set_version_label(self, box_index: int, version: int):
        state = self.box_states[box_index]
        # version이 None/0이어도 표시 갱신이 필요할 수 있어서, “버전+모델 문자열” 기준으로 비교
        ...
        vtxt = self.format_version(version)
        model = state.get("model_text")

        # 모델은 너무 길면 우측 상단 UI가 깨져서 짧게
        if model:
            model_short = model.strip()
            if len(model_short) > 10:
                model_short = model_short[:10] + "…"
            text = f"{vtxt} {model_short}"
        else:
            text = vtxt

        if state.get("last_model_text") == text:
            return
