...
if __name__ == "__main__":
    ...
    for i in range(total_boxes):
        ...
        # Ensure enough padding to avoid overlapping
        if i < len(modbus_boxes):
            modbus_ui.box_frames[i][0].grid(row=row_index, column=column_index, padx=20, pady=20)
        else:
            analog_ui.box_frames[i - len(modbus_boxes)][0].grid(row=row_index, column=column_index, padx=20, pady=20)
