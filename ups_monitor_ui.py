...
class UPSMonitorUI:
    def __init__(self, root, num_boxes):
        self.root = root
        self.box_frame = Frame(self.root, bg='black')  # 메인 프레임의 배경색을 검정으로 설정
    ...
    def create_ups_box(self, index):
        ...
        if col == 0:
            row_frame = Frame(self.box_frame, bg='black')  # 각 행의 배경색을 검정으로 설정
        ...
        box_frame = Frame(row_frame, highlightthickness=int(2 * SCALE_FACTOR), bg='black')
        ...
        box_canvas = Canvas(inner_frame, width=int(150 * SCALE_FACTOR), height=int(300 * SCALE_FACTOR), highlightthickness=int(1 * SCALE_FACTOR), highlightbackground="#000000", highlightcolor="#000000", bg='black')
        box_canvas.pack()
