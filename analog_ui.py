...
class AnalogUI:
    ...
    def _append_memory_log(self, box_index: int, entry: dict):
    ...
    def maybe_log_event(self, box_index: int, raw_mA: float, full_scale_value: float,
        ...
        last_logged_display = state.get("last_logged_display")
        last_logged_alarm1 = state.get("last_logged_alarm1")
        last_logged_alarm2 = state.get("last_logged_alarm2")
        last_logged_pwr = state.get("last_logged_pwr")

        changed = (
            last_logged_display is None
            or abs(float(normalized_display) - float(last_logged_display)) >= resolution
            or alarm1 != last_logged_alarm1
            or alarm2 != last_logged_alarm2
            or pwr_on != last_logged_pwr
            or bool(event)
        )
        ...
        if not event:
            reasons = []
            if last_logged_alarm1 is None and last_logged_alarm2 is None and last_logged_pwr is None:
                reasons.append("INIT")
            else:
                if alarm1 != last_logged_alarm1:
                    reasons.append("AL1_ON" if alarm1 else "AL1_OFF")
                if alarm2 != last_logged_alarm2:
                    reasons.append("AL2_ON" if alarm2 else "AL2_OFF")
                if pwr_on != last_logged_pwr:
                    reasons.append("PWR_ON" if pwr_on else "PWR_OFF")
                if abs(float(normalized_display) - float(last_logged_display if last_logged_display is not None else normalized_display)) >= resolution:
        ...
        memory_entry = {
            ...
            "box_index": box_index,
            "gas_type": gas_type,
            "raw_mA": round(float(raw_mA), 4),
            "display_value": display_value_str,
            "full_scale_value": round(float(full_scale_value), 3),
            "alarm1": bool(alarm1),
            "alarm2": bool(alarm2),
            "pwr_on": bool(pwr_on),
            "event": event
        }
        self._append_memory_log(box_index, memory_entry)
    ...
    def create_analog_box(self, index, initial_gas_types):
        ...
        box_canvas.create_rectangle(
            0, 0, int(160 * SCALE_FACTOR), int(200 * SCALE_FACTOR),
            fill='grey', outline='grey', tags='border'
        )
        box_canvas.create_rectangle(
            0, int(200 * SCALE_FACTOR), int(160 * SCALE_FACTOR), int(310 * SCALE_FACTOR),
    ...
    async def read_adc_values(self, adc, adc_index):
        try:
            ...
            if 0 <= adc_index * 4 < self.num_boxes:
                self.maybe_log_event(
                    box_index=adc_index * 4,
                    raw_mA=0.0,
                    full_scale_value=0.0,
                    gas_type=self.gas_types.get(f"analog_box_{adc_index * 4}", StringVar(value="ORG")).get(),
                    alarm1=False,
                    alarm2=False,
                    pwr_on=False,
